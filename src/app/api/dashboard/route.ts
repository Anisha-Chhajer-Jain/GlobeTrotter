...
import { MOCK_CITIES, MOCK_TRIPS } from "@/lib/mock-data";
...
export async function GET() {
  try {
    ...
    try {
      ...
      console.warn("[Dashboard API] DB offline, serving mock dashboard data:", dbErr);
      return jsonResponse({
        stats: {
        ...
        upcomingTrips: MOCK_TRIPS,
        pastTrips: [],
        sharedWithMe: [],
        popularCities: MOCK_CITIES,
        recommendedDestinations: MOCK_CITIES.slice(0, 4),
        expenseByCategory: [
          { category: "TRANSPORT", total: 850, count: 1 },
          { category: "ACCOMMODATION", total: 600, count: 1 },
        ],
